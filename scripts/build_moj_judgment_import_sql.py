#!/usr/bin/env python3
"""Generate the idempotent review-only import SQL for verified MOJ judgment PDFs."""
...
OUTPUT = ROOT / "imports" / "20260906_moj_five_volumes_judgments_review.sql"
...
def payload(data: list[dict[str, object]]) -> str:
    ...
    if "$archive_payload$" in text:
        raise ValueError("Unexpected SQL dollar-quote marker")
...
def main() -> None:
    ...
    if not (len(sources) == 1 and len(source_files) == 5 and len(documents) == 191 and len(document_files) == 191):
        raise SystemExit("Unexpected Ministry batch cardinality")
    if any(item.get("document_type") != "judgment" or item.get("status") != "review" or item.get("legal_search_eligibility") is not False for item in documents):
        raise SystemExit("All Ministry documents must be review-only judgments")
    if any(item.get("is_downloadable") is not False for item in document_files):
        raise SystemExit("All Ministry files must remain non-downloadable")

    source_id = str(sources[0]["id"])
    batch_id = str(uuid.uuid5(NAMESPACE, "archive-batch:moj-1434-five-volumes-review-001"))
    sql = f"""-- Generated review-only Ministry judgment data batch.
BEGIN;
...
SELECT '{batch_id}'::uuid,'{source_id}'::uuid,'moj-1434-five-volumes-review-001',now(),now(),
       5,191,191,0,191,0,0,'review',NULL,
       'Official originals and standalone PDFs verified; strict manual field review pending.'
LIMIT 1
ON CONFLICT (id) DO NOTHING;
